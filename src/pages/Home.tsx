...
export default function Home() {
    ...
    const fetchWeeklyHighlights = async () => {
      ...
      const [{ data: weeklyItems, error: weeklyError }, { data: allProducts, error: productsError }] = await Promise.all([
        supabase
          ...
          .gte('orders.created_at', sevenDaysAgo.toISOString()), // Use 7 days ago here
      ...
      setWeeklyHighlights([...topByCompletedOrders, ...randomFillers]);
    };
  ...
  const handleCheckOrder = async (event: FormEvent) => {
    ...
    if (!parsedOrderId || !normalizedPhone) {
      setCheckOrderError(t('home_check_modal_desc'));
      return;
    }
    ...
    <div className="min-h-screen bg-[#f6f7fb] text-slate-900 overflow-x-hidden">
      ...
      <section className="relative h-screen flex items-center justify-center">
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto mt-[-5vh]">
          ...
          <h1 className="text-5xl md:text-8xl font-serif text-slate-900 mb-6 leading-tight animate-fade-in-up delay-100">
            {t('home_tagline_title')} <br />
            <span className="italic text-[#C5A572]">{t('home_tagline_highlight')}</span>
          </h1>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3">
              ...
              >
                <span className="relative flex items-center justify-center gap-3 text-slate-900 uppercase tracking-widest text-xs md:text-sm group-hover:text-[#9c7a4c] transition-colors">
                  {t('home_order_now')} <ArrowRight size={16} />
                </span>
              </button>
              ...
              >
                <span className="relative flex items-center justify-center gap-3 text-slate-900 uppercase tracking-widest text-xs md:text-sm group-hover:text-[#9c7a4c] transition-colors">
                  {t('home_check_order')} <Search size={16} />
                </span>
              </button>
            </div>
      ...
      <section className="py-24 px-6 relative overflow-hidden bg-[#f6f7fb] reveal-on-scroll">
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            <div>
              <h2 className="text-[#C5A572] text-xs tracking-widest uppercase mb-2">{t('home_favorites')}</h2>
              <h3 className="text-4xl font-serif text-slate-900">{t('home_weekly')}</h3>
            </div>
            <button
              onClick={() => navigate('/menu')}
              className="flex items-center gap-2 text-sm text-slate-500 hover:text-[#9c7a4c] transition-colors"
            >
              {t('home_view_full_menu')} <ChevronRight size={16} />
            </button>
          </div>
            ...
            {displayedHighlights.map((item) => (
              ...
              >
                <div className="overflow-hidden rounded-2xl mb-4 aspect-[4/5] relative shadow-sm border border-slate-200 bg-white">
                  ...
                  {item.quantity > 0 && (
                    <div className="absolute bottom-3 right-3 z-20 bg-white/90 backdrop-blur-md text-slate-900 text-[10px] md:text-xs font-semibold px-3 py-1.5 rounded-full shadow-sm flex items-center gap-1.5 border border-white/20">
                      <span className="w-1.5 h-1.5 rounded-full bg-[#C5A572] animate-pulse" />
                      {item.quantity} {t('home_ordered_count')}
                    </div>
                  )}
      ...
      <section className="py-24 px-6 bg-white border-y border-slate-200">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-16">
          ...
          <div className="w-full md:w-1/2 text-center md:text-left">
            <h2 className="text-[#C5A572] text-xs tracking-widest uppercase mb-4">{t('home_story_label')}</h2>
            <h3 className="text-4xl md:text-5xl font-serif text-slate-900 mb-6">{t('home_story_title')}</h3>
            ...
            <div className="flex flex-col md:flex-row gap-8 justify-center md:justify-start">
              <div>
                ...
                <span className="text-xs text-slate-500 uppercase tracking-wider">{t('home_happy_customers')}</span>
              </div>
              <div>
                <span className="block text-3xl font-serif text-slate-900">12+</span>
                <span className="text-xs text-slate-500 uppercase tracking-wider">{t('home_signature_blends')}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-24 px-6 bg-[#f6f7fb] reveal-on-scroll">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-[#C5A572] text-sm tracking-[0.2em] uppercase font-sans mb-3">{t('home_visit_us')}</h2>
            <h3 className="text-4xl font-serif text-slate-900">{t('home_find_way')}</h3>
          </div>
            ...
            <div className="space-y-6 order-2 md:order-1">
              <div className="p-8 rounded-2xl bg-white border border-slate-200 shadow-sm">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <h4 className="text-xl font-serif text-slate-900">{t('home_opening_hours')}</h4>
                </div>
              ...
              <div className="p-8 rounded-2xl bg-white border border-slate-200 shadow-sm">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <h4 className="text-xl font-serif text-slate-900">{t('home_address')}</h4>
                </div>
      ...
      <footer className="bg-white pt-24 pb-12 px-6 border-t border-slate-200">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
          ...
          <div>
            <h5 className="text-slate-900 font-medium mb-6">{t('home_footer_explore')}</h5>
          ...
          <div>
            <h5 className="text-slate-900 font-medium mb-6">{t('home_footer_contact')}</h5>
          ...
          <div>
            <h5 className="text-slate-900 font-medium mb-6">{t('home_stay_connected')}</h5>
            <p className="text-slate-500 text-sm mb-4">{t('home_stay_desc')}</p>
          </div>
        </div>
      ...
      {isCheckOrderOpen && (
        <div className="fixed inset-0 z-[90] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md rounded-2xl bg-white border border-slate-200 shadow-2xl p-5 md:p-6">
            ...
            <h3 className="font-serif text-2xl text-slate-900 mb-1">{t('home_check_modal_title')}</h3>
            <p className="text-sm text-slate-500 mb-4">{t('home_check_modal_desc')}</p>

            <form onSubmit={handleCheckOrder} className="space-y-3">
              <input
                value={checkOrderId}
                onChange={(e) => setCheckOrderId(e.target.value)}
                placeholder={t('home_order_id')}
                className="w-full rounded-xl border border-slate-200 px-4 py-2.5"
              />
              <input
                value={checkPhone}
                onChange={(e) => setCheckPhone(e.target.value)}
                placeholder={t('home_phone_number')}
                className="w-full rounded-xl border border-slate-200 px-4 py-2.5"
              />
              ...
              >
                {isCheckingOrder ? t('home_checking') : t('home_open_order_detail')}
              </button>
            </form>
          </div>
        </div>
